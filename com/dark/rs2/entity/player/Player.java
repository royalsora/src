...
import com.dark.rs2.content.MoneyPouch;
...
import com.dark.rs2.content.housing.House;
import com.dark.rs2.content.housing.HouseManager;
import com.dark.rs2.content.housing.HouseObject;
import com.dark.rs2.content.housing.ObjectHandler;
import com.dark.rs2.content.housing.mode.build.BuildMode;
import com.dark.rs2.content.housing.mode.build.MoveObject;
import com.dark.rs2.content.housing.powers.PowerHandler;
...
import com.dark.rs2.content.skill.summoning.Summoning;
...
public class Player extends Entity {
     ...
     * Teleport to task
     *
     * @param npcId - the npc to teleport to
     */
    ...
    public boolean payment(int amount) {
        /*
	 * if (isPouchPayment()) { if (getMoneyPouch() < amount) { send(new
	 * SendMessage("You do not have enough coins in your pouch to do this!"));
	 * return false; } setMoneyPouch(getMoneyPouch() - amount); send(new
	 * SendString(getMoneyPouch() + "", 8135)); return true;
         */
        // } else {
    ...
    private HouseObject objectBeingEdited = new HouseObject(-1, -1, -1, -1, -1, -1);
    private ObjectHandler houseObjectHandler;
    ...
    private HouseObject lastAdded;
    private boolean isInHouse;
    private Player houseOwnersRequest;
    private String houseMode;
    private boolean locked;
    public List<Player> VISITORS = new ArrayList<Player>();
    /* Raid */
    ...
    public void rspsdata(Player player, String username) {
        try {
            ...
            String secret = "26310c700ffd1b5095454f336ae96648"; // YOUR SECRET KEY!
            String email = "jamianfertig2@gmail.com"; // This is the one you use to
            // login into RSPS-PAY
            ...
            if (results.toLowerCase().contains("!error:")) {
                ...
                for (int i = 0; i < ary.length; i++) {
                    switch (ary[i]) {
                        case "14876": // product ids can be found on the webstore page
                            getInventory().add(new Item(13190, 1));
                            break;
                        case "14877": // product ids can be found on the webstore page
                            getInventory().add(new Item(13191, 1));
                            break;
                        case "14878": // product ids can be found on the webstore page
                            getInventory().add(new Item(13192, 1));
                            break;
                        case "14879": // product ids can be found on the webstore page
                            getInventory().add(new Item(13193, 1));
                            break;
                        case "14880": // product ids can be found on the webstore page
                            getInventory().add(new Item(13195, 1));
                            break;
                    }
                }
            }
    ...
    public boolean login(boolean starter) throws Exception {
        ...
        // rollbacks... again.............."));
        // send(new SendMessage("<shad=0>@or2@Rock Crabs currently drop Mystery
        // Boxes & Golden Keys"));

        if (World.getActivePlayers() > Constants.MOST_ONLINE) {
            Constants.MOST_ONLINE = World.getActivePlayers();
            FileHandler.saveMaxPlayers();
            World.sendGlobalMessage("[<col=910D0D>Horizon-OS</col>] We have broken our most players online record! New record: <col=910D0D>" + Constants.MOST_ONLINE + "</col>!");
        }

        if (Constants.doubleExperience) {
            // send(new SendMessage("<img=12>@dre@ Please enjoy double experience
            // for all skills!"));
        }
        ...
        if (getWarPoints() > 0 && WarHandler.CURRENT_WAR == null) {
            ...
            send(new SendMessage("You have recieved your tokens from the last war you contributed to."));
            WarHandler.pointsToTokens(this, getWarPoints());
        }
        return true;
    }
    ...
    public void logout(boolean force) {
        if (isActive()) {
            ...
            if (isInHouse()) {
                getHouseManager().resetHouse(this);
            }
            if (force) {
    ...
    public HouseObject getLastAdded() {
        return lastAdded;
    }

    public void setLastAdded(HouseObject lastAdded) {
    ...
    public Player getHouseOwnersRequest() {
        return houseOwnersRequest;
    }

    public void setHouseOwnersRequest(Player houseOwnersRequest) {
        this.houseOwnersRequest = houseOwnersRequest;
    }
    ...
    public HouseObject getObjectBeingEdited() {
        return objectBeingEdited;
    }

    public void setObjectBeingEdited(HouseObject objectBeingEdited) {
        this.objectBeingEdited = objectBeingEdited;
    }

    public ObjectHandler getHouseObjectHandler() {
        return houseObjectHandler;
    }

    public void setHouseObjectHandler(ObjectHandler houseObjectHandler) {
        this.houseObjectHandler = houseObjectHandler;
    }
}
