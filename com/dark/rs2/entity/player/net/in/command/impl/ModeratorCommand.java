...
import com.dark.rs2.content.io.PlayerSave.PlayerDetails;
...
import com.dark.rs2.entity.player.net.out.impl.SendSystemBan;
...
public class ModeratorCommand implements Command {
	...
	case "modcommands":
	case "modcommand":
		player.send(new SendString("Horizon-OS Mod Command List", 8144));
		...
		if (parser.hasNext()) {
			try {
				...
				if (target == null) {
					...
					new SendSystemBan().execute(target.getClient());
					PlayerSaveUtil.setIPBanned(target);
					target.setBanned(true);
					target.setBanLength(-1);
					target.logout(true);
					player.send(new SendMessage("IPED."));
				}
			} catch (Exception e) {
		...
		if (parser.hasNext()) {
			try {
				...
				if (target == null) {
					...
					// target.setUsername(Utility.formatPlayerName(name));
					if (!PlayerDetails.loadDetails(target)) {
						// player.send(new SendMessage("The player '" +
						// Utility.formatPlayerName(name) + "' was not
						// found."));
						return true;
					}
					save = true;
