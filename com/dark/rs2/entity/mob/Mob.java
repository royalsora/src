...
import com.dark.core.definitions.NpcCombatDefinition.Ranged;
import com.dark.core.task.TaskQueue;
import com.dark.core.task.impl.MobDeathTask;
import com.dark.core.task.impl.MobWalkTask;
import com.dark.core.util.GameDefinitionLoader;
import com.dark.core.util.Utility;
import com.dark.rs2.content.combat.CombatConstants;
...
import com.dark.rs2.content.sounds.MobSounds;
...
import com.dark.rs2.entity.following.MobFollowing;
import com.dark.rs2.entity.item.Item;
import com.dark.rs2.entity.mob.impl.CorporealBeast;
//import com.dark.rs2.entity.mob.impl.EnragedCow;
import com.dark.rs2.entity.mob.impl.GiantMole;
import com.dark.rs2.entity.mob.impl.KalphiteQueen;
import com.dark.rs2.entity.mob.impl.Kraken;
import com.dark.rs2.entity.mob.impl.Kreearra;
import com.dark.rs2.entity.mob.impl.PenanceQueen;
import com.dark.rs2.entity.mob.impl.SeaTrollQueen;
import com.dark.rs2.entity.mob.impl.Tentacles;
import com.dark.rs2.entity.mob.impl.wild.Callisto;
import com.dark.rs2.entity.mob.impl.wild.ChaosElemental;
import com.dark.rs2.entity.mob.impl.wild.ChaosFanatic;
import com.dark.rs2.entity.mob.impl.wild.CrazyArchaeologist;
import com.dark.rs2.entity.mob.impl.wild.Scorpia;
import com.dark.rs2.entity.mob.impl.wild.Vetion;
import com.dark.rs2.entity.movement.MobMovementHandler;
	...
	if (npcId == 496 && getOwner().inKraken()) {
		...
		if ((npcId == 1286)) {
				//if ((hit.getAttacker() != null) && (!hit.getAttacker().isNpc())) {
					...
					Item arrows = player.getEquipment().getItems()[13];
					if (((weapon != null) && (weapon.getId() == 6522) || ((arrows != null) && (arrows.getId() == 2538)) || ((arrows != null) && (arrows.getId() == 2540)) || ((arrows != null) && (arrows.getId() == 11217)))) {
						hit.getDamage();
					} else {
						hit.setDamage(0);
						player.send(new SendMessage("<shad=0>@red@You can only damage shades with fire weapons such as Obby Rings or Fire Arrows"));
					}
				}
		...
		if ((npcId == 2910)) {
			//if ((hit.getAttacker() != null) && (!hit.getAttacker().isNpc())) {
			Player player = World.getPlayers()[hit.getAttacker().getIndex()];
				if (player.getSkill().getLevels()[Skills.HUNTER] > 92) {
					hit.getDamage();
				} else {
					hit.setDamage(0);
					player.send(new SendMessage("@dre@You need 93 Hunter to harm this monster."));
		...
		if ((npcId == 2911)) {
			//if ((hit.getAttacker() != null) && (!hit.getAttacker().isNpc())) {
				Player player = World.getPlayers()[hit.getAttacker().getIndex()];
				if (player.getSkill().getLevels()[Skills.HUNTER] > 62) {
					hit.getDamage();
				} else {
					hit.setDamage(0);
					player.send(new SendMessage("@dre@You need 63 Hunter to harm this monster."));
				}
			}
				...
				if ((npcId == 2912)) {
					//if ((hit.getAttacker() != null) && (!hit.getAttacker().isNpc())) {
						Player player = World.getPlayers()[hit.getAttacker().getIndex()];
						if (player.getSkill().getLevels()[Skills.HUNTER] > 72) {
							hit.getDamage();
						} else {
							hit.setDamage(0);
							player.send(new SendMessage("@dre@You need 73 Hunter to harm this monster."));
						}
					}
					
	if ((npcId == 2267) && ((hit.getType() == Hit.HitTypes.MELEE) || (hit.getType() == Hit.HitTypes.RANGED))) {
		hit.setDamage(0);
	}

	if ((npcId == 4067) && ((hit.getType() == Hit.HitTypes.MELEE) || (hit.getType() == Hit.HitTypes.RANGED))) {
		hit.setDamage(0);
	}

	if ((npcId == 2265) && ((hit.getType() == Hit.HitTypes.MAGIC) || (hit.getType() == Hit.HitTypes.RANGED))) {
		hit.setDamage(0);
	}

	if ((npcId == 2266) && ((hit.getType() == Hit.HitTypes.MAGIC) || (hit.getType() == Hit.HitTypes.MELEE))) {
		hit.setDamage(hit.getDamage() / 10);
	}

	if ((npcId == 7286) && ((hit.getType() == Hit.HitTypes.MAGIC) || (hit.getType() == Hit.HitTypes.RANGED))) {
		hit.setDamage(hit.getDamage() / 5);
	}
	if ((npcId == 3508) && ((hit.getType() == Hit.HitTypes.MELEE) || (hit.getType() == Hit.HitTypes.MAGIC))) {
		hit.setDamage(hit.getDamage() / 5);
	}
	if ((npcId == 4344) && ((hit.getType() == Hit.HitTypes.MELEE) || (hit.getType() == Hit.HitTypes.MAGIC))) {
		hit.setDamage(hit.getDamage() / 5);
	}
	if ((npcId == 4345) && ((hit.getType() == Hit.HitTypes.MELEE) || (hit.getType() == Hit.HitTypes.MAGIC))) {
		hit.setDamage(hit.getDamage() / 5);
	}
	if ((npcId == 4346) && ((hit.getType() == Hit.HitTypes.MELEE) || (hit.getType() == Hit.HitTypes.MAGIC))) {
		hit.setDamage(hit.getDamage() / 5);
	}
	if ((npcId == 7152) && ((hit.getType() == Hit.HitTypes.MAGIC) || (hit.getType() == Hit.HitTypes.RANGED))) {
		hit.setDamage(0);
	}
	if ((npcId == 319) && ((hit.getType() == Hit.HitTypes.MAGIC) || (hit.getType() == Hit.HitTypes.RANGED))) {
		hit.setDamage(hit.getDamage() / 5);
	}
