package com.dark.rs2.entity.mob;
...
import com.dark.core.definitions.ItemDropDefinition.ItemDropTable;
...
import com.dark.rs2.content.cluescroll.ClueDifficulty;
import com.dark.rs2.content.cluescroll.ClueScroll;
import com.dark.rs2.content.cluescroll.ClueScrollManager;
import com.dark.rs2.content.cluescroll.scroll.EmoteScroll;
...
import com.dark.rs2.entity.player.net.out.impl.SendKillFeed;
...
public class MobDrops {
	...
	if (player.isInRaid() && player.getRaidOwner() != null) {

		if (player.getRaidOwner().getRaid().listOfRoundMobs.contains(mob) && player.getRaidOwner().getRaid().playersInRaid.contains(player.getUsername())) {
			player.getRaidOwner().getRaid().listOfRoundMobs.remove(mob);
			System.out.println("Player Removed " + mob.getDefinition().getName() + " from the list");
		}

		if (player.getRaidOwner().getRaid().listOfRoundMobs.isEmpty()) {
			RoundHandler.getRoundDamage(player.getRaidOwner());
			RoundHandler.startRound(player.getRaidOwner(), player.getRaidOwner().getRaid().bossType);
		}
	...
	case 5884:
		player.getUpdateFlags().sendGraphic(Graphic.highGraphic(287, 0));
		player.hit(new Hit(20, Hit.HitTypes.POISON));
		player.poison(20);
		player.setAbyssalPoints(player.getAbyssalPoints() + 250);
		AchievementHandler.activateAchievement(player, AchievementList.KILL_50_PLAYER, 1);
		break;

	case 5918:
		player.getUpdateFlags().sendGraphic(Graphic.highGraphic(287, 0));
		player.hit(new Hit(5, Hit.HitTypes.POISON));
		player.poison(5);
		player.setAbyssalPoints(player.getAbyssalPoints() + 125);
		AchievementHandler.activateAchievement(player, AchievementList.KILL_15_PLAYER, 1);
		break;
	...
	case 5916:
		player.setAbyssalPoints(player.getAbyssalPoints() + 65);
	...
	case 1466:
		AchievementHandler.activateAchievement(player, AchievementList.KILL_1_GORILLA, 1);
		player.getProperties().addProperty(mob, 1);
		break;

	case 7152:
		AchievementHandler.activateAchievement(player, AchievementList.ENCHANT_500_BOLTS, 1);
		break;

	case 2463:
		AchievementHandler.activateAchievement(player, AchievementList.KILL_100_CYCLOPS, 1);
		break;

	case 2919:
		AchievementHandler.activateAchievement(player, AchievementList.KILL_10_MITH, 1);
		break;
	...
	case 2910: // king chinchompa
		player.hit(new Hit(10));
		player.getInventory().add(10034, 5);
		player.getInventory().add(11959, 5);
		player.getSkill().addExperience(22, 1000);
		break;

	case 2911: // red chinchompa
		player.hit(new Hit(2));
		player.getInventory().add(10034, 1);
		player.getSkill().addExperience(22, 385);
		break;

	case 2912: // black chinchompa
		player.hit(new Hit(3));
		player.getInventory().add(11959, 1);
		player.getSkill().addExperience(22, 550);
		break;
	...
	case 6401:
		AchievementHandler.activateAchievement(player, AchievementList.KILL_75_COWS, 1);
		AchievementHandler.activateAchievement(player, AchievementList.EQUIP_DRAGON_CLAWS, 1);
		break;

	/*
	 * case 2955: case 2954: player.setArenaPoints(player.getArenaPoints() + 1);
	 * player.send(new SendMessage("@dre@You now have " +
	 * player.getArenaPoints() + " Mage Arena points."));
	 * AchievementHandler.activateAchievement(player,
	 * AchievementList.EARN_100_MAGE_ARENA_POINTS, 1);
	 * AchievementHandler.activateAchievement(player,
	 * AchievementList.EARN_500_MAGE_ARENA_POINTS, 1); break;
	 */
	...
	if (mob instanceof CaveKraken) {
		dropLocation = new Location(2482, 9810);
	}
	...
	if ((entity != null) && (!entity.isNpc()) && ((p = World.getPlayers()[entity.getIndex()]) != null)) {
		...
		Item glove = p.getEquipment().getItems()[9];
		Item amulet = p.getEquipment().getItems()[2];

		if ((ring != null) && (ring.getId() == 2572)) {
			ucRoll += 5;
			rtMod = 2;
		}
		if ((ring != null) && (ring.getId() == 10872)) {
			ucRoll += 5;
			rtMod = 2;
		}
		if ((glove != null) && (glove.getId() == 1580)) {
			ucRoll += 5;
			rtMod = 2;
		}
		if ((amulet != null) && (amulet.getId() == 1499)) {
			ucRoll += 7;
			rtMod = 4;
		}
	...
	dropClues(player, mob, dropLocation)) {
		return;
	}
	...
	Item glove = player.getEquipment().getItems()[9];
	...
	/*
	 * if (!raresOnly &&
	 * player.getController().equals(ControllerManager.DEFAULT_CONTROLLER)) { if
	 * (random.nextInt(100) <= 8) { if (Utility.randomNumber(100) < 20) {
	 * items.add(new Item(6831)); } } }
	 */
	/*
	 * if (!raresOnly &&
	 * player.getController().equals(ControllerManager.DEFAULT_CONTROLLER)) { if
	 * (random.nextInt(100) <= 8) { if (Utility.randomNumber(120) < 40) {
	 * items.add(new Item(6199)); } } }
	 */
	if (drops == null) {
		return items;
	}
	...
	if (rare != null) {
		...
		int roll = random.nextInt(750 - (p != null && chance < 50 ? p.getRareDropEP().getEpAddon() >= 750 ? 600 : p.getRareDropEP().getEpAddon() : 0));
		...
		if (roll >= 500 && roll <= 500 + chance + mod) {
			...
			if (p != null) {
				if (chance < 100 || p.getRareDropEP().getReceived() >= 1) {
