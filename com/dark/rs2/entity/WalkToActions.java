...
import com.dark.core.cache.map.Doors;
import com.dark.core.cache.map.DoubleDoor;
...
import com.dark.core.task.impl.HarvestTask;
import com.dark.core.task.impl.HopDitchTask;
...
import com.dark.rs2.content.dialogue.impl.AchievementDialogue;
import com.dark.rs2.content.dialogue.impl.AdamDialogue;
import com.dark.rs2.content.dialogue.impl.BigMoDialogue;
import com.dark.rs2.content.dialogue.impl.BuyHouse;
import com.dark.rs2.content.dialogue.impl.DecantingDialogue;
import com.dark.rs2.content.dialogue.impl.DunceDialogue;
import com.dark.rs2.content.dialogue.impl.EmblemDialogue;
import com.dark.rs2.content.dialogue.impl.FiremakingDialogue;
import com.dark.rs2.content.dialogue.impl.GamblerDialogue;
import com.dark.rs2.content.dialogue.impl.GenieResetDialogue;
import com.dark.rs2.content.dialogue.impl.GwenDialogue;
import com.dark.rs2.content.dialogue.impl.HardModeDialouge;
import com.dark.rs2.content.dialogue.impl.HariDialogue;
import com.dark.rs2.content.dialogue.impl.JarDialogue;
...
import com.dark.rs2.content.dialogue.impl.OddOldMan;
import com.dark.rs2.content.dialogue.impl.PilesDialogue;
import com.dark.rs2.content.dialogue.impl.SelipDialogue;
import com.dark.rs2.content.dialogue.impl.PrestigeDialogue;
import com.dark.rs2.content.dialogue.impl.RaidCreateDialogue;
import com.dark.rs2.content.dialogue.impl.RaidJoinDialouge;
import com.dark.rs2.content.dialogue.impl.RaidStartDialouge;
import com.dark.rs2.content.dialogue.impl.RaidTutorialDialouge;
import com.dark.rs2.content.dialogue.impl.RunecraftingTeleport;
import com.dark.rs2.content.dialogue.impl.SailorDialogue;
import com.dark.rs2.content.dialogue.impl.SecurityDialogue;
...
import com.dark.rs2.content.dialogue.impl.teleport.WildernessLever;
import com.dark.rs2.content.dwarfcannon.DwarfMultiCannon;
import com.dark.rs2.content.membership.MysteryBoxMinigame;
import com.dark.rs2.content.minigames.barrows.Barrows;
import com.dark.rs2.content.minigames.clanwars.ClanWarsFFA;
import com.dark.rs2.content.minigames.fightcave.TzharrGame;
import com.dark.rs2.content.minigames.fightpits.FightPits;
import com.dark.rs2.content.minigames.godwars.GodWars;
...
//import com.dark.rs2.content.minigames.raid.MainHandler;
import com.dark.rs2.content.minigames.warriorsguild.ArmourAnimator;
...
import com.dark.rs2.content.shopping.impl.PestShop;
import com.dark.rs2.content.skill.Skills;
import com.dark.rs2.content.skill.agility.Agility;
import com.dark.rs2.content.skill.agility.obstacle.interaction.WalkInteraction;
import com.dark.rs2.content.skill.cooking.CookingTask;
import com.dark.rs2.content.skill.crafting.CraftingType;
import com.dark.rs2.content.skill.crafting.Flax;
...
import com.dark.rs2.content.skill.runecrafting.RunecraftingTask;
import com.dark.rs2.content.skill.smithing.SmithingConstants;
import com.dark.rs2.content.skill.thieving.HomeStalls;
import com.dark.rs2.content.skill.thieving.ThievingNpcTask;
import com.dark.rs2.content.skill.thieving.ThievingStallTask;
import com.dark.rs2.content.skill.thieving.WallSafes;
import com.dark.rs2.content.skill.woodcutting.WoodcuttingTask;
import com.dark.rs2.content.wilderness.Lockpick;
...
import com.dark.rs2.entity.player.net.out.impl.SendChatBoxInterface;
...
import com.dark.rs2.entity.player.net.out.impl.SendItemOnInterface;
...
public class WalkToActions {
     ...
     */
    public static final int[][] NPC_STORE_DATA = {{519, 0}, {518, 0}, // General
    ...
    public static void clickObject(final Player player, final int option, final int id, final int x, final int y) {
        ...
        if ((id == 1815) && x == 3154 && y == 3294 && player.getX() == x && player.getY() == y) {
        ...
        if (id == 27984) {
            player.teleport(new Location(1753, 3854, 0));
            player.send(new SendMessage("You climb over the rocks."));
        }
        if (id == 27987) {
            player.teleport(new Location(1769, 3849, 0));
            player.send(new SendMessage("You climb over the rocks."));
        }
        if (id == 27988) {
            player.teleport(new Location(1774, 3849, 0));
            player.send(new SendMessage("You climb over the rocks."));
        }
        if (id == 26377) { // sara climb rocks
            player.teleport(new Location(2918, 5274, 0));
            player.send(new SendMessage("You climb over the rocks."));
        }
        if (id == 26766) { // crevice
            player.teleport(new Location(3034, 10158, 0));
            player.send(new SendMessage("You go through the crevice"));
        }
        if (id == 26769) { // crevice
            player.teleport(new Location(3017, 3740, 0));
            player.send(new SendMessage("You go through the crevice"));
        }
        if (id == 5798) { // gorilla
            // player.teleport(new Location(2715, 9169, 1));
            player.getMagic().teleportNoWildernessRequirement(2715, 9169, 1, MagicSkill.TeleportTypes.SPELL_BOOK);
            player.send(new SendMessage("You enter the dark crevice."));
        }
        if (id == 26761) { // edgeville lever
            player.teleport(new Location(3153, 3923, 0));
            player.send(new SendMessage("You pull the lever, bringing you to the deep wilderness."));
        }
        if (id == 26194) { // edgeville lever
            player.teleport(new Location(3090, 3475, 0));
        ...
        if (id == 27980) {
            ...
            player.getSkill().addExperience(20, amount * 3.5);
            player.getInventory().add(566, amount);
        }
        if (id == 27978) {
            int amount = player.getEssenceFragments();
            player.incrementEssenseFragment(-amount);
            player.getSkill().addExperience(20, amount * 2.5);
            player.getInventory().add(565, amount);
        }
        if (id == 21725) {
            player.teleport(new Location(2636, 9510, 2));
            player.send(new SendMessage("You climb up the stairs."));
        }
        if (id == 21726) {
            player.teleport(new Location(2636, 9517, 0));
            player.send(new SendMessage("You climb down the stairs."));
        }
        if (id == 4133) {
            if (!player.getInventory().hasItemId(3460)) {
                DialogueManager.sendItem1(player, "You do not have the required key.", 3460);
                return;
            }
            player.getInventory().remove(3460, 1);
            player.teleport(new Location(3513, 9696, 0));
        }
        if (id == 4132) {
            if (!player.getInventory().hasItemId(3460)) {
                DialogueManager.sendItem1(player, "You do not have the required key.", 3460);
        ...
        if (id == 8689) {
            if (!player.getInventory().hasItemId(1925)) {
                DialogueManager.sendItem1(player, "A bucket is required to do this!", 1925);
                return;
            }
            ...
            player.send(new SendMessage("You manage to get some milk."));
        ...
        if (id == 27555 && player.getInventory().hasItemId(2347)) {
            if (System.currentTimeMillis() - player.diceDelay >= 1500) {
                player.diceDelay = System.currentTimeMillis();
                player.getSkill().addExperience(12, 40);
                player.getInventory().remove(1539, 10);
                player.getInventory().remove(960, 1);
                player.getUpdateFlags().sendAnimation(new Animation(898));

            }

        }

        if (id == 27557 && player.getInventory().hasItemId(952)) {
            player.getUpdateFlags().sendAnimation(new Animation(830));
            player.getInventory().add(13431, 1);
        }
        int ITEMS[][] = {{13445}};
        int ITEMS2[][] = {{13446}};
        for (int i = 0; i < ITEMS.length; i++) {
            ...
            if (id == 27979) {
                ...
                player.getInventory().add(new Item(ITEMS2[i][0], amount));
    ...
    public static final void finishClickNpc(Player player, int option, Mob mob) {
        ...
        switch (option) {
            ...
            case 1:// NPC FIRST CLICK
                ...
                switch (id) {
                    case 1353:
                        player.start(new BuyHouse(player));
                        break;
                    case 356:
                        player.start(new HardModeDialouge(player));
                        break;
                    case 3936:// Sailor
                    ...
                    case 3063:// hunding
                        player.teleport(new Location(2552, 3556, 0));
                        break;

                    case 5529:// chinchompa
                        player.getMagic().teleport(2503, 2893, 0, MagicSkill.TeleportTypes.SPELL_BOOK);
                        player.send(new SendMessage("You need a @red@63@bla@ hunter to catch red chinchompas."));
                        break;
                    ...
                    case 765:
                        if (MainHandler.raidList.contains(player.getUsername()) && player.getRaid().partySize > 3) {
                            player.start(new RaidStartDialouge(player));
                            return;
                        }
                        player.start(new RaidJoinDialouge(player));
                        break;
                    case 1011:// Gambler
                    ...
                    case 2183:// Oziach
                        ...
                        break;
                    case 1617:// Gwen
                        player.start(new GwenDialogue(player));
                        break;
                    case 954:// Barrows
                    ...
                    case 13:// Piles
                        ...
                        break;
                    case 1615:// Piles
                        player.start(new SelipDialogue(player));
                        break;
                    case 3772:// crazy old man
                        player.start(new JarDialogue(player));
                        break;
                    case 403:// Slayer
                    ...
                    case 326:// Genie reset
                        ...
                        break;
                    case 1259:// odd old
                        player.start(new OddOldMan(player));
                        break;
                    case 3951:// Bank Pin
            ...
            case 2:// NPC SECOND CLICK
                ...
                switch (id) {
                    case 765: // raid create

                        player.start(new RaidCreateDialogue(player));
                        break;
                    case 2195:// William
            ...
            case 3:// NPC THIRD CLICK
                ...
                switch (id) {
                    case 765:
                        player.start(new RaidTutorialDialouge(player));
                        break;
                    case 1103:// Weapon Game
                    ...
                    case 4936:// Mage of zamorak
                        player.start(new OptionDialogue("Abyss", p -> {
                            ...
                            // TaskQueue.queue(new TeleOtherTask(mob, player, new
                            // Location(2923, 4819)));
                        }));
                        break;
                    case 1325:// Npc guide
    ...
    public static final void finishItemOnNpc(Player player, int item, Mob mob) {
        switch (mob.getId()) {
            ...
            /*
	 * case 1011: //use coins on gambler
	 * 
	 ...
	 * 
	 * if (Utility.random(100) >= 55) { player.getInventory().add(995, amount *
	 * 2); DialogueManager.sendItem1(player,
	 * "You have just managed to double your money.", 995); } return;
             */
    ...
    public static void finishObjectClick(Player player, int id, int option, int x, int y) {
        ...
        switch (option) {
            case 1:
                ...
                if (id == 8720 || id == 26820 || id == 26796 || id == 26809 || id == 26813) {
                    //	player.send(new SendString("http://www.dark-asylum.org/", 12000));
                ...
                switch (id) {
                    ...
                    case 28861:
                        player.getBank().openBank();
                        break;
                    case 3757:// doantor cave
                        player.teleport(new Location(3555, 9947));
                        break;
                    ...
                    case 12941:// Membership Fountain
                        ...
                        break;
                    case 26760:// Wilderness resource trivia
                        if (player.getLocation().getX() == 3184 && player.getLocation().getY() == 3945) {
                            if (player.isPouchPayment()) {
                                if (player.getMoneyPouch() < 7500) {
                                    DialogueManager.sendStatement(player, "You need 7,500 coins to enter the trivia.");
                                    return;
                                }
                            } else {
                            ...
                            player.send(new SendMessage("You have paid 7,500 coins and entered the resource trivia."));
                            ...
                            TaskQueue.queue(new WalkThroughDoorTask(player, x, y, z, new Location(2998, 3931, z)) {
                                ...
                                public void execute() {
                                    if (stage == 0) {
                                        ...
                                        ObjectManager.getInstance().unregister(new GameObject(door.getId(), door.getX(), door.getY(), door.getZ(), door.getType(), door.getCurrentFace()));
                                        ...
                                        p.getClient().queueOutgoingPacket(new SendSound(326, 0, 0));
                                        ObjectManager.getInstance().unregister(new GameObject(normalDoor.getCurrentId1(), normalDoor.getX1(), normalDoor.getY1(), normalDoor.getZ(), normalDoor.getType(), normalDoor.getCurrentFace1()));
                                        ObjectManager.getInstance().unregister(new GameObject(normalDoor.getCurrentId2(), normalDoor.getX2(), normalDoor.getY2(), normalDoor.getZ(), normalDoor.getType(), normalDoor.getCurrentFace2()));
                                        normalDoor.append();
                                        stage++;
                                    } else if (stage == 6) {
                        ...
                        if (player.getLocation().getY() >= 3931) {
                            TaskQueue.queue(new WalkThroughDoubleDoorTask(player, x, y, z, new Location(2998, 3931, z)) {
                                ...
                                public void execute() {
                                    if (stage == 0) {
                                        p.getClient().queueOutgoingPacket(new SendSound(326, 0, 0));
                                        ObjectManager.getInstance().unregister(new GameObject(ObjectManager.BLANK_OBJECT_ID, door.getX1(), door.getY1(), door.getZ(), door.getType(), door.getCurrentFace1()));
                                        ObjectManager.getInstance().unregister(new GameObject(ObjectManager.BLANK_OBJECT_ID, door.getX2(), door.getY2(), door.getZ(), door.getType(), door.getCurrentFace2()));
                                        door.append();
                                        ...
                                        p.getClient().queueOutgoingPacket(new SendSound(326, 0, 0));
                                        ObjectManager.getInstance().unregister(new GameObject(ObjectManager.BLANK_OBJECT_ID, normalDoor.getX(), normalDoor.getY(), normalDoor.getZ(), normalDoor.getType(), normalDoor.getCurrentFace()));
                                        normalDoor.append();
                                        ObjectManager.getInstance().registerWithoutClipping(new GameObject(normalDoor.getCurrentId(), normalDoor.getX(), normalDoor.getY(), normalDoor.getZ(), normalDoor.getType(), normalDoor.getCurrentFace()));
                                        stage++;
                                    } else if (stage == 6) {
                                        p.getMovementHandler().walkTo(0, -1);
                                        stage++;
                                    } else if (stage == 7) {
                                        ObjectManager.getInstance().registerWithoutClipping(new GameObject(ObjectManager.BLANK_OBJECT_ID, normalDoor.getX(), normalDoor.getY(), normalDoor.getZ(), normalDoor.getType(), normalDoor.getCurrentFace()));
                                        normalDoor.append();
                                        ObjectManager.getInstance().registerWithoutClipping(new GameObject(normalDoor.getCurrentId(), normalDoor.getX(), normalDoor.getY(), normalDoor.getZ(), normalDoor.getType(), normalDoor.getCurrentFace()));
                                        stop();
                                        return;
                    ...
                    case 10439:
                        player.resetLevels();
                        player.curePoison(0);
                        player.getClient().queueOutgoingPacket(new SendMessage("The spirit of the desert Blesses you!"));
                        break;
                    ...
                    case 13291:
                        if (player.getInventory().contains(new Item(2944))) {
                            RareChest.searchChest(player, x, y);
                        } else {
                            player.send(new SendMessage("You will need a Chest key to open this."));
                        }
                        break;
                    case 9319:
                        player.changeZ(1);
    ...
    public static void itemOnObject(final Player player, final int itemId, final int objectId, final int x, final int y) {
        ...
        TaskQueue.queue(new WalkToTask(player, x, y, length[0], length[1]) {
            ...
            public void onDestination() {
                ...
                /*
		 * if (Item.getDefinition(itemId).isNote() && objectId == 10517) { int
		 * note_amount = player.getInventory().getItemAmount(itemId); int
		 * freeSlots = player.getInventory().getFreeSlots(); int
		 * notedSubtraction = note_amount - freeSlots;
		 * player.getInventory().remove(new Item(itemId, note_amount));
		 * player.getInventory().add(new Item(itemId, notedSubtraction));
		 * player.getInventory().add(new Item(itemId, freeSlots));
		 * player.getInventory().update();
		 * 
		 * }
                 */
 /*
		 * if (objectId == 10517 && Item.getDefinition(itemId).isNote()) { int
		 * note_amount = player.getInventory().getItemAmount(itemId);
		 * System.out.println("You have " + note_amount + " of this item."); int
		 * freeSlots = player.getInventory().getFreeSlots();
		 * System.out.println("You should recieve " + freeSlots +
		 * " unnoted items."); int notedSubtraction = note_amount - freeSlots;
		 * System.out.println("You have " + notedSubtraction +
		 * " noteditems left.");
		 * 
		 * player.getInventory().remove(new Item(itemId, note_amount));
		 * player.getInventory().add(new Item(itemId, notedSubtraction));
		 * player.getInventory().add(new Item(itemId - 1, freeSlots));
		 * 
		 * player.getInventory().update(); return; }
                 */
                if (objectId == 10517 && Item.getDefinition(itemId).isNote()) {
                    int notedAmount = player.getInventory().getItemAmount(itemId);
                    int freeSlots = player.getInventory().getFreeSlots();
                    // 5 items 25 free slots i should only get 5 unnoted.
                    int newNotedAmount = notedAmount - freeSlots;
                    int unnoteAmount = notedAmount - newNotedAmount;

                    if (notedAmount <= freeSlots) {
                        player.getInventory().remove(itemId, player.getInventory().getItemAmount(itemId));
                        player.getInventory().add(new Item(itemId - 1, notedAmount));
                        player.getInventory().update();
                        return;
                    }
                    if (notedAmount >= freeSlots) {
                        player.getInventory().remove(itemId, player.getInventory().getItemAmount(itemId));
                        player.getInventory().add(itemId, newNotedAmount);
                        player.getInventory().add(new Item(itemId - 1, unnoteAmount));
                        player.getInventory().update();
                        return;
                    }
                ...
                // platinum tokens
                if (objectId == 11744 && itemId == 995) {
                    player.start(new tokensDialogue(player));

                }
                if (objectId == 11744 && itemId == 13204) {
                    player.start(new tokensDialogue2(player));
                }

                if (BoneBurying.useBonesOnAltar(player, itemId, objectId)) {
                    return;
                }
                if (player.getBuildMode().itemOnObject(player, itemId, objectId))// House
                {
                    return;
                }
                if (objectId == 26254) {
                ...
                if (objectId == 27979 && itemId == 13446) {
                    if (player.getInventory().hasItemId(7938)) {
                        return;

                    }
                    player.getInventory().add(7938, 1);
