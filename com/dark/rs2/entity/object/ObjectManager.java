...
public class ObjectManager {
    ...
    private static final void deleteWithObject(int x, int y, int z) {
        ...
        if (object == null) {
            if (z > 0) {
                ObjectManager.getInstance().register(new GameObject(2376, x, y, z, 10, 0, true));
            }
            return;
        }
        ...
        if ((object.getType() != 10) || (z > 0)) {
            ObjectManager.getInstance().registerWithoutClipping(new GameObject(2376, x, y, z, object.getType(), object.getFace(), true));
        } else {
            ObjectManager.getInstance().register(new GameObject(2376, x, y, z, object.getType(), object.getFace(), true));
        }
    }
