...
import com.dark.rs2.content.skill.ranged.RangedConstants;
...
import com.dark.rs2.entity.item.EquipmentConstants.WeaponAttackStyles;
...
public class Equipment {
		...
		if (requirements != null) {
			...
			for (byte[] requirement : requirements) {
				if (player.getMaxLevels()[requirement[0]] < requirement[1]) {
					String name = com.dark.rs2.content.skill.Skills.SKILL_NAMES[requirement[0]];
	...
	public void equip(Item item, int clickSlot) {
		...
		if (item.getId() == 6583) {
			stoneRing.init(player);
		}

		if (item.getId() == 12931) {
			player.poison(0);
			player.getClient().queueOutgoingPacket(new SendMessage("<shad=0>@gre@The poison enters your bloodstream giving you immunity."));	
