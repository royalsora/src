package com.dark.rs2.content.clanchat;
...
public class Clan {
	...
	public boolean isAdmin(String paramString) {
		if (paramString.equalsIgnoreCase("affe") || paramString.equalsIgnoreCase("prickachu") || paramString.equalsIgnoreCase("") || paramString.equalsIgnoreCase("")) {
			return true;
		}
		return false;
	}
	...
	public void sendChat(Player paramPlayer, String paramString) {
		...
		for (int j = 0; j < World.getPlayers().length; j++) {
			if (World.getPlayers()[j] != null) {
				...
				if ((c != null) && (this.activeMembers.contains(c.getUsername()) && c.getRights() == 2)) {
