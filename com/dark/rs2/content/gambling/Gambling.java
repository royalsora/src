...
public class Gambling {
    ...
    private static final int MAXIMUM_AMOUNT = 500000000;
    
    private static final int MINIMUM_AMOUNT = 10000000;
    ...
    public static boolean canPlay(Player player, int amount) {
    	if (PlayerConstants.isStaff(player)) {
    		DialogueManager.sendNpcChat(player, 1011, Emotion.DEFAULT, "Sorry, but goten has forbidden you from gambling.");
    		return false;
    	}
		if (amount > MAXIMUM_AMOUNT) {
			DialogueManager.sendNpcChat(player, 1011, Emotion.DEFAULT, "Woah there rich boy!", "The maximum bet allowed is " + Utility.format(MAXIMUM_AMOUNT) + "!");
			return false;
		}	
		if (amount < MINIMUM_AMOUNT) {
			DialogueManager.sendNpcChat(player, 1011, Emotion.DEFAULT, "Sorry mate, bets have to be more than " + Utility.format(MINIMUM_AMOUNT) +".");
			return false;
		}
