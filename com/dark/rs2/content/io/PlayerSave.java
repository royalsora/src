package com.dark.rs2.content.io;
...
import com.dark.rs2.entity.Entity.AttackType;
...
public final class PlayerSave {
	...
	public static final class PlayerDetails {
		...
		try {
			...
			player.setAbyssalPoints(details.abyssalPoints);
			...
			player.setWarPoints(details.warPoints);
			...
			player.setHouse(details.playerHouse);
			...
			player.setDicer(details.isDicer);
			player.setIron(details.isIron);
			player.setHardMode(details.isHard);
			...
			player.setEssenceFragments(details.essAmount);
		...
		private final int abyssalPoints;
		private final int teleblockTime;
		private final int blackMarks;
		private final House playerHouse;
		...
		isDicer = player.isDicer();
		isIron = player.isIron();
		isHard = player.isHardMode();
		essAmount = player.getEssenceFragments();
		...
		abyssalPoints = player.getAbyssalPoints();

		soundVolume = player.getSoundVolume();

		deaths = player.getDeaths();
		kills = player.getKills();
		warPoints = player.getWarPoints();
		...
		lastLoginYear = Player.LastLoginYear;
		LastLoginMonth = Player.LastLoginMonth;
