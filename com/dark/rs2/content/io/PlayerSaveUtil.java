...
public class PlayerSaveUtil {
	...
	public static final boolean isBanned(Player p) {
	BufferedReader r = null;
	try {
		r = new BufferedReader(new FileReader("./data/logs/bans.txt"));
		String l = null;

		while ((l = r.readLine()) != null) {
			if (l.contains(p.getUsername()))
				return true;
		}
	} catch (Exception e) {
		e.printStackTrace();
	...
	public static final void setBanned(Player p, int length) {
	BufferedWriter bw = null;
	try {
		bw = new BufferedWriter(new FileWriter(new File("./data/logs/bans.txt"), true));
		bw.newLine();
