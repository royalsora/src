package com.dark.rs2.content.moderation;
...
import com.dark.rs2.entity.player.net.out.impl.SendInventory;
import com.dark.rs2.entity.player.net.out.impl.SendInventoryInterface;
...
public class StaffTab {
	...
	public static boolean createTarget(Player player, String name, int... rights) {
		...
		if (target == null) {
			...
			try {
				if (!PlayerContainer.loadDetails(target)) {
					player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] Player <col=255>" + name + "</col> does not exist!"));
					return false;
				}
			} catch (Exception e) {
				e.printStackTrace();
		...
		if (PlayerConstants.isHighClass(target) && !player.getUsername().equalsIgnoreCase("goten")) {
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You may not punish <col=255>" + name + "</col>!"));			
			return false;
		}
		...
		if (!access) {
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You do not have access to this!"));	
			return false;
		}
	...
	public static void handle(Player player, String user, PunishmentType punishment, int... rights) {
		...
		case CHECK_BANK:
			...
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You are now viewing <col=255>" + name + "</col>'s bank!"));			
			break;
			
		case KICK:
			target.logout(true);
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have successfully kicked <col=255>" + name + "</col>!"));
			break;
			
		case MUTE:
			target.setMuted(true);
			target.setMuteLength(System.currentTimeMillis() + 1 * 3_600_000);
			target.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have been muted for 1 hour!"));
			PlayerSave.save(target);
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have successfully muted <col=255>" + name + "</col> for 1 hour!"));			
			break;
			
		case UNMUTE:
			target.setMuted(false);
			target.setMuteLength(0);
			target.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have been unmuted!"));
			PlayerSave.save(target);
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have successfully unmuted <col=255>" + name + "</col>!"));			
			break;
			
		case BAN:
			target.setBanned(true);
			target.setBanLength(System.currentTimeMillis() + 1 * 3_600_000);
			target.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have been banned for 1 hour!"));
			PlayerSave.save(target);
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have successfully banned <col=255>" + name + "</col> for 1 hour!"));			
			break;
			
		case UNBAN:
			target.setBanned(false);
			target.setBanLength(0);
			target.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have been unbanned!"));
			PlayerSave.save(target);
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have successfully unbanned <col=255>" + name + "</col>!"));				
			break;
		...
		case JAIL:
			...
			target.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have been jailed for 1 hour!"));
			PlayerSave.save(target);
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have successfully jailed <col=255>" + name + "</col> for 1 hour!"));				
			break;
		...
		case UNJAIL:
			...
			target.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have been unjailed!"));
			PlayerSave.save(target);
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have successfully unjailed <col=255>" + name + "</col>!"));				
			break;
			
		case MOVE_HOME:
			target.teleport(PlayerConstants.HOME);
			target.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have been moved home!"));
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have successfully moved <col=255>" + name + "</col>!"));							
			break;
		...
		case COPY:
    		...
    		player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have successfully copied <col=255>" + name + "</col>!"));							 		
			break;
			
		case FREEZE:
			target.freeze(10, 5);
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have successfully froze <col=255>" + name + "</col>!"));							 				
			break;
		...
		case INFO:
			...
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You are now viewing <col=255>" + name + "</col>'s account info!"));							 						
			break;
		...
		case GIVE_MODERATOR:
			...
			target.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have been given moderator status by " + player.getUsername() + "!"));
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have given <col=255>" + name + "</col> moderator status."));							 								
			break;
			
		case KILL:
			target.hit(new Hit(target.getSkill().getLevels()[3], HitTypes.DISEASE));
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have killed <col=255>" + name + "</col>!"));							 									
			break;
			
		case TELETO:
			player.teleport(target.getLocation());
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have teleported to <col=255>" + name + "</col>!"));							 									
			break;
			
		case TELETOME:
			target.teleport(player.getLocation());
			target.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have been teleported to <col=255>" + player.getUsername() + "</col>!"));							 											
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have teleported <col=255>" + name + "</col> to your location!"));							 									
			break;
			
		case BOO:
			target.send(new SendInterface(18681));
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have boo'd <col=255>" + name + "</col>!"));							 											
			break;
		...
		case RANDOM_NPC:
			...
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have transformed <col=255>" + name + "</col> into NPC <col=255>" + npcDef.getName() + " </col>(<col=255>" + npcDef.getId() + "</col>)!"));							 											
			break;
		...
		case REFRESH:
			...
			player.send(new SendMessage("[ <col=255>Horizon-OS</col> ] You have refreshed <col=255>" + name + "</col>!"));							 											
			break;
