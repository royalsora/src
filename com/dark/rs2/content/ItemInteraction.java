	...
	public static boolean clickPouch(Player p, int id, int option) {
		if (id == 5509 || id == 5510 || id == 5512 || id == 5514) {
			if (option == 1) {
				...
				p.getInventory().getItemAmount(1436);
				int invAmount1 = p.getInventory().getItemAmount(7936);
				if (invAmount1 == 0) {
					p.getClient().queueOutgoingPacket(new SendMessage("You do not have any essence to fill your pouch with."));
					return true;
				}
				if (amount > invAmount1) {
					amount = invAmount1;
				}
				...
				//p.getInventory().add(1436, p.getPouches()[index]);
				int add1 = p.getInventory().add(7936, p.getPouches()[index]);
				p.getPouches()[index] = ((byte) (p.getPouches()[index] - add1));
