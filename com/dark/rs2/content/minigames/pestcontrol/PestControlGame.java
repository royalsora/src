...
import com.dark.rs2.content.minigames.pestcontrol.monsters.Portal;
...
public class PestControlGame {
		...
		for (Player p : players) {
			...
			if (success) {
				if (p.getAttributes().get(PEST_DAMAGE_KEY) != null && p.getAttributes().getInt(PEST_DAMAGE_KEY) >= 80) {
					...
					p.getInventory().addOrCreateGroundItem(995, p.getAttributes().getInt(PEST_DAMAGE_KEY) * 1000, true);
					p.setPestPoints(p.getPestPoints() + (15));
					AchievementHandler.activateAchievement(p, AchievementList.EARN_100_PEST_CONTROL_POINTS, 1);
					AchievementHandler.activateAchievement(p, AchievementList.EARN_500_PEST_CONTROL_POINTS, 1);
	...
	public void init() {
		...
		voidKnight.getLevels()[Skills.DEFENCE] = 100;
	...
	public void process() {
		...
		for (Player p : players) {
			...
			if (p.getAttributes().get(PEST_DAMAGE_KEY) != null) {
				...
				p.getClient().queueOutgoingPacket(new SendString((damage >= 50 ? "" : "@red@") + p.getAttributes().getInt(PEST_DAMAGE_KEY), 21116));
			}
		}
	}
