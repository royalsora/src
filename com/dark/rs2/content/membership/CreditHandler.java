...
public enum CreditHandler {
	...
	public void handle(Player player) {
		...
		player.send(new SendMessage("You have been directed to the Gold Points store."));
	}
}),
	RESTORE_RUN(205051, 1, new Handle() {
	@Override
	public void handle(Player player) {
		if (!allowed(player, null, 1)) {
			return;
		...
		player.send(new SendMessage("You have just restored your run energy!"));
		spent(player, 1);
	}
}),
	...
	public static boolean allowed(Player player, CreditPurchase credit, int amount) {
		...
		if (player.getCredits() < amount) {
			DialogueManager.sendStatement(player, "@red@You do not have enough Gold points to do this!");
			player.send(new SendMessage("Please visit @red@https://www.Horizon-OS.org/ </col>to purchase more Gold points!"));
			return false;
		}
	...
	public static void spent(Player player, int amount) {
		player.send(new SendMessage("@blu@You have spent " + amount + " Gold points; Remaing: " + player.getCredits() + "."));
		player.getClient().queueOutgoingPacket(new SendString("</col>Gold points: @gre@" + Utility.format(player.getCredits()), 52504));	
