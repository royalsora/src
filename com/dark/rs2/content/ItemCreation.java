...
import com.dark.rs2.content.dialogue.impl.ConfirmDialogue;
...
public enum ItemCreation {
	...
	/* Holy Book */
	HOLY_BOOK(new int[] { 3827, 3828,  3829, 3830, 3839}, 3840, new CreationHandle() {
	@Override
	public void handle(Player player, ItemCreation data) {
		if (meetsRequirements(player, data)) {
			player.start(new ConfirmDialogue(player) {
	...
	/* Slayer Helmet */
	SLAYER_HELMET(new int[] { 4166, 8921, 4164, 4168, 4551, 4155}, 11864, new CreationHandle() {
	@Override
	public void handle(Player player, ItemCreation data) {
	if (meetsRequirements(player, data)) {
		player.start(new ConfirmDialogue(player) {
			...
			public void onConfirm() {
				if (meetsRequirements(player, data)) {
					if (!skillRequired(player, Skills.SLAYER, 90)) {
						return;
					}
						ItemDefinition item = GameDefinitionLoader.getItemDef(data.getNewItem());
						DialogueManager.sendItem1(player, "You have created a @dre@" + item.getName() + "</col>.", data.getNewItem());
	...
	/* Dragon SQ Shield */
	DRAGON_SQ_SHIELD(new int[] { 2366, 2368 }, 1187, new CreationHandle() {
		@Override
		public void handle(Player player, ItemCreation data) {
			if (meetsRequirements(player, data)) {
				player.start(new ConfirmDialogue(player) {
