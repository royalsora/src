...
import com.dark.rs2.content.skill.magic.spells.SuperHeat;
import com.dark.rs2.content.skill.magic.spells.Vengeance;
...
public class MagicSkill {
    ...
    public boolean clickMagicItems(int id) {
        switch (id) {
            ...
            case 11741:
                if (canTeleport(TeleportTypes.TABLET)) {
                    player.getInventory().remove(id, 1);
                    teleport(2956, 3217, 0, TeleportTypes.TABLET);
                    return true;
                }
                break;
            case 11742:
                if (canTeleport(TeleportTypes.TABLET)) {
                    player.getInventory().remove(id, 1);
                    teleport(2896, 3455, 0, TeleportTypes.TABLET);
                    return true;
                }
                break;
            case 11743:
                if (canTeleport(TeleportTypes.TABLET)) {
                    player.getInventory().remove(id, 1);
                    teleport(3351, 3003, 0, TeleportTypes.TABLET);
                    return true;
                }
                break;
            case 11745:
                if (canTeleport(TeleportTypes.TABLET)) {
                    player.getInventory().remove(id, 1);
                    teleport(2788, 3178, 0, TeleportTypes.TABLET);
                    return true;
                }
                break;
            case 11746:
                if (canTeleport(TeleportTypes.TABLET)) {
                    player.getInventory().remove(id, 1);
                    teleport(2604, 3093, 0, TeleportTypes.TABLET);
                    return true;
                }
                break;
            case 8014:
        ...
        switch (type) {
            ...
            default:
                switch (spellBookType) {
                    case ANCIENT:
                        PlayerSounds.sendRegionSound(player, player.getLocation(), 1012, 6, 0);
                        //player.getClient().queueOutgoingPacket(new SendSound(1012, 6, 0));
                    ...
                    case LUNAR:
                        ...
                        break;
                    default:
                        PlayerSounds.sendRegionSound(player, player.getLocation(), 202, 1, 0);
    ...
    private void initTeleport(final int x, final int y, final int z, final TeleportTypes type) {
        ...
        if (player.getRaidOwner() != null && player.isInRaid() && player == player.getRaidOwner()) {
            player.getRaidOwner().getRaid().resetRaid(player.getRaidOwner());
            return;
        } else if (player.getRaidOwner() != null && player.isInRaid()) {
            player.teleport(new Location(PlayerConstants.HOME));
            player.getRaidOwner().getRaid().playersInRaid.remove(player.getUsername());
        }
        teleporting = true;
        player.setTakeDamage(false);
        player.getClient().queueOutgoingPacket(new SendRemoveInterfaces());
    ...
    public boolean needsRune(int runeId) {
        ...
        switch (runeId) {
            case 556:
                if ((wep == 1381) || (wep == 1397) || (wep == 11998) || (wep == 17293)) {
                    return false;
                }
                return true;
            case 554:
                if ((wep == 1387) || (wep == 1393) || (wep == 11787) || (wep == 11998) || (wep == 12795) || (wep == 3053)) {
                    return false;
                }
                return true;
            case 555:
                if ((wep == 1383) || (wep == 1395) || (wep == 11787) || (wep == 12795) || (wep == 17293) || ((shield != null) && (shield.getId() == 18346))) {
                    return false;
                }
                return true;
            case 557:
