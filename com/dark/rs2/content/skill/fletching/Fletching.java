package com.dark.rs2.content.skill.fletching;
...
public enum Fletching {
    ...
    public boolean itemOnItem(Player player, Item use, Item with) {
        ...
        if (!(use.getId() == fletchable.getUse().getId() && with.getId() == fletchable.getWith().getId())
                && !(use.getId() == fletchable.getWith().getId() && with.getId() == fletchable.getUse().getId())) {
            return false;
