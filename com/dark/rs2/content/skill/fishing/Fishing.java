...
public class Fishing {
	...
	public static enum FishingSpots {
		...
		ANGLERFISH(2653, new FishableData.Fishable[] { FishableData.Fishable.ANGLERFISH }, new FishableData.Fishable[] { FishableData.Fishable.ANGLERFISH }),
		...
		if (success(f)) {
			...
			if (Utility.random(4000) == 1) {
				...
				if (petDrop != null) {
					...
					GroundItemHandler.add(new Item(13320, 1), player.getLocation(), player, player.ironPlayer() ? player : null);
				}
			}
			
			if (player.getLevels()[Skills.FISHING] <= 50) {
				if (Utility.random(20) == 1) {
					int REWARD = Utility.random(25_000);
					player.getInventory().addOrCreateGroundItem(995, REWARD, true);
				player.send(new SendMessage("You receive an extra reward while Fishing."));
			}
			}
			if (player.getLevels()[Skills.FISHING] >= 51 && player.getLevels()[Skills.FISHING] <= 90) {
			...
			if (player.getLevels()[Skills.FISHING] >= 91 && player.getLevels()[Skills.FISHING] <= 99) {
			...
			if (player.getEquipment().contains(9799) || player.getEquipment().contains(9798)) {
				if (Utility.random(50) == 1) {
				player.getInventory().add(new Item(id, 1));
				player.send(new SendMessage("You manage to catch another fish because you're a master in fishing."));
			}
			}
			else {
