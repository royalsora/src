package com.dark.rs2.content.skill.farming;
...
public class Herbs {
	...
	public enum HerbFieldsData {
		ARDOUGNE(0, new Point[] { new Point(2675, 3374), new Point(2675, 3375) }),
	...
	public boolean harvest(int objectX, int objectY) {
		...
		TaskQueue.queue(new Task(player, 3, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.FARMING) {
			...
			public void execute() {
				...
				if (player.getLevels()[Skills.FARMING] <= 50) {
				...
				if (player.getLevels()[Skills.FARMING] >= 51 && player.getLevels()[Skills.FARMING] <= 90) {
				...
				if (player.getLevels()[Skills.FARMING] >= 91 && player.getLevels()[Skills.FARMING] <= 99) {
				...
				if (player.getEquipment().contains(9811) || player.getEquipment().contains(9810)) {
					if (Utility.random(5) == 1) {
					player.getInventory().add(herbData.getHarvestId(), 2);
					player.send(new SendMessage("You find another herb next to the one you just dug up."));
					}
				}
