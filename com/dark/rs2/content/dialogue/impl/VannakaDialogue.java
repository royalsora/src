...
public class VannakaDialogue extends Dialogue {
	...
	public boolean clickButton(int id) {
		...
		case DialogueConstants.OPTIONS_4_3:
			...
			} else if (!player.getInventory().hasItemAmount(995, 100000)) {
				DialogueManager.sendStatement(player, new String[] { "@blu@100,000</col> coins is required to do this;", "which you do not have!" });
				end();
			} else {
				player.getSlayer().reset();
				DialogueManager.sendStatement(player, new String[] { "You have reset your task for @blu@100,000 </col>coins." });
				player.getInventory().remove(995, 100000, true);
	...
	public void execute() {
		...
		case 2:
			...
			//DialogueManager.setSocialSlayerPartner;
