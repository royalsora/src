...
import com.dark.core.task.impl.PlayerDeathTask;
...
import com.dark.rs2.content.combat.formula.MagicFormulas;
import com.dark.rs2.content.combat.formula.MeleeFormulas;
import com.dark.rs2.content.combat.formula.RangeFormulas;
import com.dark.rs2.content.combat.impl.PoisonWeapons;
import com.dark.rs2.content.combat.impl.RingOfRecoil;
import com.dark.rs2.content.combat.impl.EliteRing;
...
import com.dark.rs2.content.skill.melee.BarrowsSpecials;
import com.dark.rs2.content.skill.prayer.PrayerBook.Prayer;
import com.dark.rs2.content.skill.ranged.BoltSpecials;
...
//fgf
	...
	if (hit.getAttacker() != null) {
		...
		EliteRing.doRRecoil(player, hit.getAttacker(), hit.getDamage());
	...
	if (!attack.isNpc()) {
		...
		// player.send(new
		// SendEntityFeed(attack.getMob().getDefinition().getName(),
		// attack.getLevels()[3] - hit, attack.getMaxLevels()[3]));
	}
	...
	if (player.getRaidOwner() != null && player.isInRaid()) {
		player.setRaidContribution(player.getRaidContribution() + player.getLastDamageDealt());
		System.out.println(player.getUsername() + " has contributed " + player.getRaidContribution() + " damage");
	}
	}
